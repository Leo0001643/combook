...
public class AppMemberService implements IAppMemberService {
    ...
    public MemberVO getMyProfile() {
        return MemberVO.from(requireCurrentMember());
    }
    ...
    private CbMember requireCurrentMember() {
        Long memberId = MemberContext.currentId();
        return Optional.ofNullable(cbMemberService.getById(memberId))
                .orElseThrow(() -> new BusinessException(CbCodeEnum.MEMBER_NOT_FOUND));
    }
}
