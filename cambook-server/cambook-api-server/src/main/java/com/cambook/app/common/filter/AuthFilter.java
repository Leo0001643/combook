...
public class AuthFilter extends OncePerRequestFilter {
    ...
    public AuthFilter(JwtUtils jwtUtils, IPermissionService permissionService, TokenKickService tokenKickService) {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (StringUtils.isNotBlank(token)) {
                ...
                if (claims != null && !isKicked(claims)) {
    ...
    /**
     * 检查 Token 是否已被踢出。
     * 若 Token 的签发时间 (iat) 小于等于 Redis 中记录的踢出时间戳，则视为失效。
     */
    private boolean isKicked(Claims claims) {
        try {
            Object rawUid   = claims.get("uid");
            String userType = claims.get("userType", String.class);
            if (rawUid == null || userType == null) return false;
            Long uid = ((Number) rawUid).longValue();
            long kickTs = tokenKickService.getKickTimestamp(userType, uid);
            if (kickTs == 0L) return false;
            long iat = claims.getIssuedAt() != null ? claims.getIssuedAt().getTime() / 1000 : 0L;
            return iat <= kickTs;
        } catch (Exception e) {
            log.warn("[Auth] isKicked check failed: {}", e.getMessage());
            return false;
        }
    }
