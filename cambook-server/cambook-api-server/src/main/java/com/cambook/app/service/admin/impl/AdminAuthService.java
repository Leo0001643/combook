...
public class AdminAuthService implements IAdminAuthService {
    ...
    public LoginVO login(AdminLoginDTO dto) {
        SysUser user = Optional.ofNullable(
                sysUserService.lambdaQuery()
                        .eq(SysUser::getUsername, dto.getUsername())
                        .last("LIMIT 1")
                        .one())
                .orElseThrow(() -> new BusinessException(CbCodeEnum.ACCOUNT_NOT_FOUND));
        ...
        if (user.getStatus() != null && user.getStatus() == CommonStatus.DISABLED.getCode()) throw new BusinessException(CbCodeEnum.ACCOUNT_BANNED);
