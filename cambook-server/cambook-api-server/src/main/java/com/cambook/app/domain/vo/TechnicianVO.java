...
public class TechnicianVO {
    ...
    // ── 登录会话信息（管理端列表专用，来自 Redis 实时查询）─────────────────────

    @Schema(description = "APP 登录状态：0=未登录 1=已登录（Token 有效期内）")
    private Integer loginStatus;

    @Schema(description = "最近登录时间（Unix 秒），未登录时为 null")
    private Long lastLoginTime;

    @Schema(description = "最近登录设备（iOS / Android / Web / Unknown）")
    ...
    public static TechnicianVO from(CbTechnician t) {
        ...
        vo.setOnlineStatus(b2i(t.getOnlineStatus()));
        vo.setAuditStatus(b2i(t.getAuditStatus()));
        vo.setRejectReason(t.getRejectReason());
        vo.setLang(t.getLang());
        vo.setIntroZh(t.getIntroZh());
        vo.setAge(b2i(t.getAge()));
        vo.setHeight(t.getHeight() == null ? null : t.getHeight().intValue());
        ...
        vo.setSettlementMode(b2i(t.getSettlementMode()));
        vo.setCommissionType(b2i(t.getCommissionType()));
        ...
        vo.setIsFeatured(b2i(t.getIsFeatured()));
        vo.setStatus(b2i(t.getStatus()));
        return vo;
    }
    ...
    private static Integer b2i(Byte b) { return b == null ? null : b.intValue(); }
}
