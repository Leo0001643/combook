...
public class OnlineUserController {
    ...
    public Result<List<OnlineUserVO>> list(@RequestParam(required = false) String username) {
        List<OnlineUserVO> all = sessionService.listAll();
        if (StringUtils.isNotBlank(username)) {
            all = all.stream().filter(u -> u.getUsername() != null && u.getUsername().contains(username)).collect(Collectors.toList());
        }
