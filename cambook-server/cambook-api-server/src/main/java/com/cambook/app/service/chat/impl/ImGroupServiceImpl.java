...
public class ImGroupServiceImpl extends ServiceImpl<ImGroupMapper, ImGroup>
    ...
    public Long createGroup(String ownerType, Long ownerId, ImCreateGroupDTO dto) {
        ...
        group.setName(dto.getName()); group.setAvatar(dto.getAvatar());
        group.setDescription(dto.getDescription()); group.setOwnerType(ownerType); group.setOwnerId(ownerId);
        group.setMemberCount(1 + dto.getMembers().size()); group.setMaxMember(500);
        group.setStatus((byte) 0); group.setCreateTime(now); group.setUpdateTime(now);
        save(group);
    ...
    public void joinGroup(Long groupId, String userType, Long userId) {
        if (memberMapper.exists(new LambdaQueryWrapper<ImGroupMember>()
            .eq(ImGroupMember::getGroupId, groupId).eq(ImGroupMember::getUserType, userType)
            .eq(ImGroupMember::getUserId, userId).eq(ImGroupMember::getStatus, 0))) return;

        long now = DateUtils.nowSecond();
        insertMember(groupId, userType, userId, (byte) 0, now);
        lambdaUpdate().eq(ImGroup::getId, groupId)
            .setSql("member_count = member_count + 1").set(ImGroup::getUpdateTime, now).update();
    }

    @Override
    @Transactional
    public void quitGroup(Long groupId, String userType, Long userId) {
        memberMapper.update(null, new LambdaUpdateWrapper<ImGroupMember>()
            .eq(ImGroupMember::getGroupId, groupId).eq(ImGroupMember::getUserType, userType)
            .eq(ImGroupMember::getUserId, userId)
            .set(ImGroupMember::getStatus, 1).set(ImGroupMember::getUpdateTime, DateUtils.nowSecond()));
        lambdaUpdate().eq(ImGroup::getId, groupId)
            .setSql("member_count = GREATEST(member_count - 1, 0)")
            .set(ImGroup::getUpdateTime, DateUtils.nowSecond()).update();
    }

    @Override
    @Transactional
    public void dismissGroup(Long groupId, String ownerType, Long ownerId) {
        ImGroup group = Optional.ofNullable(getById(groupId))
        ...
        lambdaUpdate().eq(ImGroup::getId, groupId)
            .set(ImGroup::getStatus, 1).set(ImGroup::getUpdateTime, DateUtils.nowSecond()).update();
    ...
    public ImGroupVO getGroupInfo(Long groupId) {
        return Optional.ofNullable(getById(groupId)).map(group -> {
            List<ImGroupMember> members = memberMapper.selectList(new LambdaQueryWrapper<ImGroupMember>()
    ...
    public List<ImGroupVO> listMyGroups(String userType, Long userId) {
        return memberMapper.selectList(new LambdaQueryWrapper<ImGroupMember>()
            .eq(ImGroupMember::getUserType, userType).eq(ImGroupMember::getUserId, userId)
            .eq(ImGroupMember::getStatus, 0))
            .stream().map(e -> Optional.ofNullable(getById(e.getGroupId())).map(g -> toVO(g, null)).orElse(null))
    ...
    private void insertMember(Long groupId, String userType, Long userId, byte role, long now) {
        ...
        m.setGroupId(groupId); m.setUserType(userType); m.setUserId(userId);
        m.setRole(role); m.setIsMuted((byte) 0); m.setStatus((byte) 0);
    ...
    private ImGroupVO toVO(ImGroup group, List<ImGroupMember> members) {
        ...
        vo.setId(group.getId()); vo.setName(group.getName()); vo.setAvatar(group.getAvatar());
        vo.setDescription(group.getDescription()); vo.setOwnerType(group.getOwnerType());
        vo.setOwnerId(group.getOwnerId()); vo.setMemberCount(group.getMemberCount());
        vo.setMaxMember(group.getMaxMember()); vo.setCreateTime(group.getCreateTime());
        if (members != null) {
            vo.setMembers(members.stream().map(m -> {
                ImGroupVO.MemberItem i = new ImGroupVO.MemberItem();
                i.setUserType(m.getUserType()); i.setUserId(m.getUserId());
                i.setRole(m.getRole()); i.setGroupAlias(m.getGroupAlias());
                return i;
