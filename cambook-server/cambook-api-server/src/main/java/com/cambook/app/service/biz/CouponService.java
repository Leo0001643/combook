...
public class CouponService {
    ...
    private final ICbCouponTemplateService cbCouponTemplateService;

    /** 分页列表 */
    public PageResult<CbCouponTemplate> pageList(Long merchantId, int page, int size, String keyword, Integer type, Integer status) {
        var p = cbCouponTemplateService.lambdaQuery()
    ...
    /** 新增 */
    public void add(Long merchantId, String nameZh, String nameEn, Integer type, BigDecimal value, BigDecimal minAmount, Integer totalCount, Integer validDays, Long startTime, Long endTime) {
        CbCouponTemplate c = new CbCouponTemplate();
        c.setMerchantId(merchantId);
        c.setNameZh(nameZh);
        c.setNameEn(nameEn);
        c.setType(type == null ? null : type.byteValue());
        c.setValue(value);
        ...
        c.setStatus(CommonStatus.ENABLED.byteCode());
        cbCouponTemplateService.save(c);
    }

    /** 编辑 */
    public void edit(Long merchantId, Long id, String nameZh, String nameEn, Integer type, BigDecimal value, BigDecimal minAmount, Integer totalCount, Integer validDays, Long startTime, Long endTime, Integer status) {
        CbCouponTemplate c = getAndVerify(id, merchantId);
        if (nameZh     != null) c.setNameZh(nameZh);
        if (nameEn     != null) c.setNameEn(nameEn);
        if (type       != null) c.setType(type == null ? null : type.byteValue());
        ...
        if (status     != null) c.setStatus(status.byteValue());
        cbCouponTemplateService.updateById(c);
    }

    /** 更新状态 */
    public void updateStatus(Long merchantId, Long id, Integer status) {
        CbCouponTemplate c = getAndVerify(id, merchantId);
        c.setStatus(status == null ? null : status.byteValue());
        cbCouponTemplateService.updateById(c);
    }
    ...
    public void delete(Long merchantId, Long id) {
        ...
        cbCouponTemplateService.removeById(id);
    }

    // ── private ──────────────────────────────────────────────────────────────

    private CbCouponTemplate getAndVerify(Long id, Long merchantId) {
        CbCouponTemplate c = Optional.ofNullable(cbCouponTemplateService.getById(id))
