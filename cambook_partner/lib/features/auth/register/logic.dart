...
class RegisterLogic extends GetxController {
  ...
  @override
  void onClose() {
    nicknameCtrl.dispose();   phoneCtrl.dispose();
    passCtrl.dispose();       confirmCtrl.dispose();
    telegramCtrl.dispose();   merchantCodeCtrl.dispose();
    super.onClose();
  }
  ...
  bool _validate() {
    final l = gL10n;
    if (nicknameCtrl.text.trim().isEmpty) { ToastUtil.warning(l.fullNameRequired); return false; }
  ...
  bool _isValidPhone(String p) =>
      RegExp(r'^\d{6,15}$').hasMatch(p.replaceAll(RegExp(r'[\s\-]'), ''));
